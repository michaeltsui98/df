 ...
 * [Orm_Database_Query] and [Orm_Database_Query_Builder] objects, which can be easily
...
abstract class Orm_Database {
	...
	public static function instance($name = NULL, array $config = NULL)
	{
		if ($name === NULL)
		{
			...
			$name = Orm_Database::$default;
		}

		if ( ! isset(Orm_Database::$instances[$name]))
		{
		    
		    $regName = "_db_{$name}";
		    if (!$db = Cola::getReg($regName)) {
		        $config_obj = Cola::config();
		        $config = (array) $config_obj->get($name);
			...
			Orm_Database::$instances[$name] = $db;
		}

		return Orm_Database::$instances[$name];
	}
	...
	{
		unset(Orm_Database::$instances[$this->_instance]);
	 ...
	 * @return  object   Orm_Database_Result for SELECT queries
	 ...
	 *          $db->commit();
	 *     }
	 *     catch (Orm_Database_Exception $e)
	 *     {
	...
	public function count_records($table)
	{
		...
		return $this->query(Orm_Database::SELECT, 'SELECT COUNT(*) AS total_row_count FROM '.$table, FALSE)
			->get('total_row_count');
	}
	 ...
	 * [Orm_Database_Expression] objects will be compiled.
	 * [Orm_Database_Query] objects will be compiled and converted to a sub-query.
	...
	public function quote($value)
	{
		...
		{
			if ($value instanceof Orm_Database_Query)
			{
				// Create a sub-query
				return '('.$value->compile($this).')';
			}
			elseif ($value instanceof Orm_Database_Expression)
			{
	...
	public function quote_column($column)
	{
		...
		if ($column instanceof Orm_Database_Query)
		{
			// Create a sub-query
			$column = '('.$column->compile($this).')';
		}
		elseif ($column instanceof Orm_Database_Expression)
		{
			// Compile the expression
			$column = $column->compile($this);
		}
		else
		{
			...
			elseif (strpos($column, '.') !== FALSE)
			{
				...
				if (($prefix = $this->table_prefix())!==NULL)
				{
	...
	public function quote_table($table)
	{
		...
		if ($table instanceof Orm_Database_Query)
		{
			// Create a sub-query
			$table = '('.$table->compile($this).')';
		}
		elseif ($table instanceof Orm_Database_Expression)
		{
			// Compile the expression
			$table = $table->compile($this);
		}
