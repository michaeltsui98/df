function flashupload(uploadid, name, textareaid, funcName, args, module, catid, authkey) {
	...
	var setting = '/index.php/contents/swfupload'+module+'&catid='+catid+'&authkey='+authkey;
	window.top.art.dialog({title:name,id:uploadid,iframe:'index.php/contents/swfupload/'+args+setting,width:'500',height:'420'}, function(){ if(funcName) {funcName.apply(this,[uploadid,textareaid]);}else {submit_ckeditor(uploadid,textareaid);}}, function(){window.top.art.dialog({id:uploadid}).close()});
}
