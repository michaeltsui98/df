...
class Models_Admin_Menu extends Cola_Model
{
    ...
    public function getMenu ($parentid, $with_self = 0)
    {
        try {
            ...
            $array = array();
            foreach ($result as $v) {
                // echo "SELECT * FROM role_priv WHERE c='{$v['c']}' AND
                // a='{$v['a']}' AND roleid={$_SESSION['user']['roleid']} <br
                // />";
                $r = $this->sql(
                        "SELECT * FROM sys_purview WHERE c='{$v['c']}' AND a='{$v['a']}' AND group_id={$_SESSION['user']['roleid']}");
                if ($r)
                    $array[] = $v;
            }
            return $array;
